...
pub struct HandleDiagnostic {
    ...
    pub handles_with_multiple_ids: i32,
...
impl AttachmentDiagnostic {
    ...
    pub fn no_file_located(&self) -> usize {
        self.missing_files
            .saturating_sub(self.no_path_provided as usize)
    }
    ...
    pub fn missing_percent(&self) -> Option<f64> {
        if self.total_attachments > 0 {
            Some(self.missing_files as f64 / f64::from(self.total_attachments) * 100.0)
        } else {
            None
        }
    }
