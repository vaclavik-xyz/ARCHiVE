...
pub fn run_pdf_export(config: &Config) -> Result<(), RuntimeError> {
    ...
    for html in &html_files {
        ...
        match html_to_pdf(&chrome.launcher, html, &pdf, &user_data_dir) {
            Ok(()) => {
    ...
    // The throwaway browser profile is no longer needed.
    let _ = remove_dir_all(&user_data_dir);
...
fn html_to_pdf(
    ...
    let status = Command::new(launcher)
        ...
        .arg("--virtual-time-budget=600000")
    ...
    match status {
        ...
        Ok(s) => Err(format!("{launcher} exited with {s}")),
        Err(why) => Err(format!("could not launch {launcher}: {why}")),
    }
}
